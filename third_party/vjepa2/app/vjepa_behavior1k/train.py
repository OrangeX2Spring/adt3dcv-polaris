...
from app.vjepa_behavior1k.behavior1k import init_behavior_data
from app.vjepa_behavior1k.transforms import make_transforms
from app.vjepa_behavior1k.utils import init_opt, init_video_model, load_checkpoint, load_pretrained
...
def main(args, resume_preempt=False):
    ...
    camera_views = cfgs_data.get("camera_views", ["head"])
    ...
    (unsupervised_loader, unsupervised_sampler) = init_behavior_data(
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def load_clips():
                ...
                actions = sample[1].to(device, dtype=torch.float, non_blocking=True)  # [B T-1 23]
                states = sample[2].to(device, dtype=torch.float, non_blocking=True)  # [B T 23]
                extrinsics = sample[3].to(device, dtype=torch.float, non_blocking=True)  # [B T 21]
